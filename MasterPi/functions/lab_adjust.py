...
# 手机APP的颜色阈值调试功能(the app's color threshold adjustment function)
...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓(iterate through all contours)
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 10:  # 只有在面积大于10时，最大面积的轮廓才是有效的，以过滤干扰(Only the contour with the largest area, which is greater than 10, is considered valid to filter out the interference.)
                area_max_contour = c
