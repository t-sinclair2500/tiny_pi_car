...
class MainWindow(QtWidgets.QWidget, Ui_Form):
    def __init__(self):
        ...
        self.tableWidget.setSelectionBehavior(QAbstractItemView.SelectRows)  # 设置选中整行，若不设置默认选中单元格(Set to select the entire row. If not, it selects the cell by default.)
        ...
        # 滑竿同步对应文本框的数值,及滑竿控制相应舵机转动与valuechange函数绑定(Synchronize the slider value with the corresponding text box, and bind the slider control to corresponding servo rotation with the 'valuechange' function.)
        ...
        # tableWidget点击获取定位的信号与icon_position函数（添加运行图标）绑定(Bind the positioning signal obtained by clicking the 'tableWidget' to the 'icon_position' function (adding a running icon))
        ...
        # 将编辑动作组的按钮点击时的信号与button_editaction_clicked函数绑定(Bind the signal when the button for editing action groups is clicked to the 'button_editaction_clicked' function.)
        ...
        # 将运行及停止运行按钮点击的信号与button_runonline函数绑定(Bind the signal when the run or stop button is clicked to the 'button_runonline' function.)
...
#             self.horizontalSlider_16.setValue(0) 
    ...
    # 选项卡选择标签状态，获取对应舵机数值(Select the tab and its label status to get the corresponding servo value.)
    ...
    # 往tableWidget表格添加一行数据的函数(add the function of a row of data to the 'tableWidget')
    ...
    # 在定位行添加运行图标按钮(add a running icon button to the specified row)
    ...
    # 编辑动作组按钮点击事件(button click event of the action group editing)
    def button_editaction_clicked(self, name):
        ...
        if name == 'insertAction':    # 插入动作
            ...
            self.tableWidget.insertRow(item)       # 插入一行(insert one low)
        ...
        for i in range(self.tableWidget.rowCount()):    #刷新编号值(refresh the numbering value)
    ...
    # 在线运行按钮点击事件(the button click event for online running)
                 ...
                 if self.Button_Run.text() == '运行' or self.Button_Run.text() == 'Run':
                    ...
                    if self.checkBox.isChecked():
                        for i in range(self.tableWidget.rowCount() - self.row):
                            ...
                            self.timer.start(int(s))       # 设置计时间隔并启动(Set the counting interval and startup.)
    ...
    # 文件打开及保存按钮点击事件(button click event for opening and saving the file)
    def button_flie_operate(self, name):
        try:            
            if name == 'openActionGroup':
                ...
                openfile = dig_o.getOpenFileName(self, 'OpenFile', '/home/pi/MasterPi/action_groups', 'd6a Flies(*.d6a)')
            ...
            if name == 'tandemActionGroup':
                ...
                openfile = dig_t.getOpenFileName(self, 'OpenFile', '/home/pi/MasterPi/action_groups', 'd6a Flies(*.d6a)')
                # 打开单个文件(open a single file)
                # 参数一：设置父组件；参数二：QFileDialog的标题(parameter 1: set the parent component; parameter 2: the title of 'QFileDialog')
                # 参数三：默认打开的目录，“.”点表示程序运行目录，/表示当前盘符根目录(parameter 3: default opened directory, '.' represents the program running directory, and '/' represents the root directory of the current disk)
                # 参数四：对话框的文件扩展名过滤器Filter，比如使用 Image files(*.jpg *.gif) 表示只能显示扩展名为.jpg或者.gif文件(parameter 4: File extension filter for the dialog box, for example, using 'Image files (.jpg *.gif)' to display only files with the extension '.jpg' or '.gif')
