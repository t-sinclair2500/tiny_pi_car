...
#将图形的像素坐标转换为机械臂的坐标系(Convert the pixel coordinates of a graphic to the coordinate system of a robotic arm.)
#传入坐标及图像分辨率，例如(100, 100, (640, 320))(the coordinates and image resolution is transmitted in, for example (100, 100, (640, 320)))
...
#将现实世界的长度转换为图像像素长度(Convert actual length to pixel length in an image.)
#传入坐标及图像分辨率，例如(10, (640, 320))(the coordinates and image resolution is transmitted in, for example (10, (640, 320)))
...
# 获取检测物体的roi区域(get the ROI area of a detected object)
# 传入cv2.boxPoints(rect)返回的四个顶点的值，返回极值点(Input the four vertex values returned by cv2.boxPoints(rect), return the extreme points.)
def getROI(box):
...
#除roi区域外全部变成黑色(Convert the rest of an image to black except for the ROI region)
#传入图形，roi区域，图形分辨率(Input the image, ROI area, and image resolution.)
...
# 获取木块中心坐标(Get the center coordinates of the block.)
# 传入minAreaRect函数返回的rect对象， 木快极值点， 图像分辨率， 木块边长(Input the rect object returned by the "minAreaRect" function, the extreme points of the block, the image resolution, and the length of the block.)
def getCenter(rect, roi, size, square_length):
    x_min, x_max, y_min, y_max = roi
    #根据木块中心的坐标，来选取最靠近图像中心的顶点，作为计算准确中心的基准(Select the vertex closest to the center of the image based on the center coordinates of the block.)
    ...
    #计算木块的对角线长度(calculate the diagonal length of the block)
...
# 获取旋转的角度(get the rotation angle)
# 参数：机械臂末端坐标, 木块旋转角(Parameters: end effector coordinates of the robotic arm, and rotation angle of the block)
